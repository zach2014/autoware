#  Copyright 2008-2014 AUTO Networks
#
...
"""Module that adds directories needed by AUTO to sys.path when imported."""
...
AUTODIR = dirname(abspath(__file__))
...
# When, for example, auto/run.py is executed as a script, the directory
# containing the auto module is not added to sys.path automatically but
# the auto directory itself is. Former is added to allow importing
# the module and the latter removed to prevent accidentally importing
# internal modules directly.
add_path(dirname(AUTODIR))
remove_path(AUTODIR)

# Default library search locations.
add_path(join(AUTODIR, 'libraries'))
add_path('.', end=True)
